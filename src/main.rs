...
struct Cli {
    ...
    /// Enable output caching for supported commands
...
enum Commands {
    ...
    Mix {
        /// Mix arguments (e.g. phx.routes, test, ecto.migrate)
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },

    /// IEx session with rtk tracking
    Iex {
        /// IEx arguments
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Mix { args } => {
            mix_cmd::run(&args, cli.verbose, cli.cache)?;
        }

        Commands::Iex { args } => {
            let timer = tracking::TimedExecution::start();
            let status = std::process::Command::new("iex")
                .args(&args)
                .status()
                .context("Failed to run iex")?;
            timer.track_passthrough(
                &format!("iex {}", args.join(" ")),
                &format!("rtk iex {}", args.join(" ")),
            );
            if !status.success() {
                std::process::exit(status.code().unwrap_or(1));
            }
