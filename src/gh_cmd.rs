...
fn list_prs(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<()> {
    ...
    // If the user supplied their own --json or --jq flags, pass through directly.
    let has_user_json = args.iter().any(|a| a == "--json" || a == "--jq");
    if has_user_json {
        return run_passthrough("gh", "pr", &{
...
fn view_pr(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<()> {
    ...
    // If the user supplied their own --json or --jq flags, pass through
    // to gh directly so we don't conflict with our own --json fields.
    let has_user_json = args[1..].iter().any(|a| a == "--json" || a == "--jq");
    if has_user_json {
        return run_passthrough("gh", "pr", &{
            let mut full = vec!["view".to_string()];
            full.extend(args.iter().cloned());
            full
        });
    }
    ...
    // Pass through additional flags (e.g., -R for repo)
    for arg in &args[1..] {
        cmd.arg(arg);
    }
...
fn pr_checks(args: &[String], _verbose: u8, _ultra_compact: bool) -> Result<()> {
    ...
    // but still writes valid output to stdout. Only treat as a real
    // error if stdout is empty and stderr has content.
...
fn view_run(args: &[String], _verbose: u8) -> Result<()> {
    ...
    // Pass through additional flags (e.g., --log-failed, -R)
    let mut cmd = Command::new("gh");
    ...
    // gh run view exits non-zero when the run failed, but still
    // writes valid output to stdout. Only treat as a real error
    // if stdout is empty and stderr has content.
    if !output.status.success() && raw.trim().is_empty() {
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();
        timer.track(
    ...
    // When --log or --log-failed is passed, the output is raw CI logs
    // rather than the structured job summary — pass it through directly.
    let has_log_flag = args[1..]
        .iter()
        .any(|a| a == "--log" || a == "--log-failed");

    let stdout = String::from_utf8_lossy(&output.stdout);
    let mut filtered = String::new();

    if has_log_flag {
        // Raw log output — print as-is
        let header = format!("🏃 Workflow Run #{}\n", run_id);
        filtered.push_str(&header);
        print!("{}", header);

        if stdout.trim().is_empty() {
            let msg = format!(
                "  ❌ To see what failed, try: gh run view {} --log-failed\n",
                run_id
            );
            filtered.push_str(&msg);
            print!("{}", msg);
        } else {
            filtered.push_str(&stdout);
            print!("{}", stdout);
        }
    } else {
        // Parse structured output and show only failures
